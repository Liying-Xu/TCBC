...
def show_train_hist(hist, show = False, save = False, path = 'Train_hist.png'):
    ...
    y3 = hist['D_loss']
    y4 = hist['G_loss']
    y5 = hist['Addition_Loss']
    y6 = hist['Identity_Loss']
    y7 = hist['Separation_Loss']
    y8 = hist['Gan_Loss']


    plt.figure(figsize=(10,5))#(长，高)
    plt.plot(x, y3, label='D_loss')
    plt.plot(x, y4, label='G_loss')
    plt.plot(x, y5, label='addition_loss')
    plt.plot(x, y6, label='identity_loss')
    plt.plot(x, y7, label='separation_loss')
    plt.plot(x, y8, label='gan_loss')
...
def generate_animation(root, model, opt):
    ...
    for e in range(opt.train_epoch):
        img_name = root + 'Fixed_results/' + model + str(e + 1) + '.tif'
        images.append(tifffile.imread(img_name))
